...
class Game extends Component {
  ...
  async componentWillMount() {
    ...
    if (type === "html") {
      await this.setState({
        ...this.state,
        javascriptQuizQuestions: this.getHtmlQuestion()
      });
    } else if (type === "iq") {
      await this.setState({
        ...this.state,
        javascriptQuizQuestions: this.getIqQuestion()
      });
    } else if (type === "gk") {
      await this.setState({
        ...this.state,
        javascriptQuizQuestions: this.getGkQuestion()
      });
    } else if (type === "sport") {
      await this.setState({
        ...this.state,
        javascriptQuizQuestions: this.getSportsQuestion()
      });
    }
    ...
    if (this.state.javascriptQuizQuestions.length) {
      this.setState({
        ...this.setState,
        javascriptQuizQuestion: {
          ...this.state.javascriptQuizQuestions[this.state.indexOfQuestion]
        }
      });
    }
  }
  ...
  getIqQuestion() {
    return [
      {
        question:
          "Which number should come next in this series 10,17,26 ______?",
      ...
      {
        question:
          "Maria has taken 6 exams and her average marks so far are 89. If she gets 100, a perfect score on the remaining 2 exams. What will her new average be?",
      ...
      {
        question:
          "It is Khadijah’s birthday today. One year from now she will be twice as old as she was 10 years ago. Calculate the present age of Khadijah ?",
  ...
  selectedOption(answer) {
    this.state.javascriptQuizQuestions[
    ...
    return (
      <div className="QuizArea">
            ...
            <div className="choice">
              ...
              {this.state.next ? (
                <button className="InstBtn">
                  <Link to="/win-screen">Proceed</Link>
                </button>
              ) : (
                <button
                  className="BtnQuiz"
        ...
        <VideoModal
          ...
          correctAnswerVideo={this.state.correctAnswerVideo}
        ></VideoModal>
      </div>
    );
  }
